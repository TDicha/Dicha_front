    ...
    <section>
      <div className="flex items-end justify-between gap-4">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-sm text-[var(--text-muted)]">구현된 주요 화면으로 바로 이동해 볼 수 있어요.</p>
        </div>
      </div>
        ...
        {quickLinks.map((item) => (
          <Link key={item.to} className="min-w-0" to={item.to}>
            <AppCard className="h-full rounded-[1.25rem]">
              <p className="break-keep text-sm font-semibold leading-5 text-[var(--brand-primary)]">{item.label}</p>
              <p className="mt-2 break-keep text-xs leading-5 text-[var(--text-muted)]">{item.description}</p>
            </AppCard>
          </Link>
        ))}
      </div>
