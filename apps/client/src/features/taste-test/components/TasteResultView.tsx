...
function getRoastResultTitle(roastLevel: string) {
  if (roastLevel === "Light") return "산뜻한 라이트 로스트";
  if (roastLevel === "Dark") return "묵직한 다크 로스트";
  return "균형감 있는 미디엄 로스트";
}

function getSpectrumRows(preference: RoastPreference) {
  return [
    {
      label: "산미",
      value: preference.acidity,
      widthClassName:
