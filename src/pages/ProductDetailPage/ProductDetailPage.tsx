...
function SectionRow({
  ...
  return (
    <button
      ...
      type="button"
    >
      <span className="text-[0.95rem] font-medium text-[var(--palette-121212)]">
        {label}
      </span>
      <span className="flex items-center gap-3">
        <span className="text-[0.92rem] font-semibold text-[var(--primary-color)]">
          {value}
        </span>
...
export function ProductDetailPage() {
  ...
  const reviewTotal = product.reviewCount ?? detail.reviews.length;
  ...
  const selectedSummary = selectedOption
    ...
    : `${detail.defaultRoastLabel} · ${detail.baseWeightLabel}`;
  ...
  return (
    <div className="min-h-screen bg-[var(--background-color)] pb-32">
      <header className="sticky top-0 z-30 flex h-14 items-center justify-between border-b border-[var(--line-color)] bg-white px-4">
        <button
          aria-label="뒤로가기"
          className="flex size-8 items-center justify-center text-[var(--palette-121212)]"
          onClick={() => navigate(-1)}
          type="button"
        >
        ...
        <Link
          aria-label="장바구니로 이동"
          className="relative flex size-8 items-center justify-center text-[var(--palette-121212)]"
          to={ROUTES.cart}
        >
          <ShoppingCart className="size-5" />
          <span className="absolute -right-1 -top-1 flex size-3.5 items-center justify-center rounded-full bg-[var(--danger-color)] text-[0.45rem] font-bold text-white">
            {itemCount}
          </span>
        </Link>
      </header>
      ...
      <section className="relative flex min-h-[24.4rem] flex-col items-center justify-center overflow-hidden bg-[var(--palette-dae3d1)] px-6 text-center">
        ...
        <p className="mt-4 text-[0.72rem] text-[var(--primary-color)]">
          DICHA × {product.name}
        </p>
      ...
      <section className="border-b border-[var(--line-color)] bg-white px-4 py-4">
        ...
        <p className="mt-1 text-[0.82rem] text-[var(--palette-666666)]">
          {product.subtitle}
        </p>
        ...
        <div className="mt-3 flex items-end gap-2">
          ...
          <p className="pb-1 text-[0.7rem] text-[var(--palette-666666)]">
            {detail.baseWeightLabel} 기준
          </p>
        </div>
        ...
        <div className="mt-1 flex items-center gap-2">
          ...
          <p className="text-[0.78rem] text-[var(--palette-666666)]">
            리뷰 {reviewTotal}건 · 판매 {detail.salesCount.toLocaleString()}건
          </p>
        </div>
      </section>

      <section className="border-b border-[var(--line-color)] bg-white px-4 py-5">
        <h2 className="text-[1.1rem] font-black text-[var(--palette-121212)]">
          커스텀 로스팅 옵션
        </h2>
        ...
        <button
          ...
          type="button"
        >
          <span className="text-[0.92rem] font-bold text-[var(--primary-color)]">
            ✨ 매장 추천 커스텀 적용
          </span>
          <span className="text-[0.74rem] font-medium text-[var(--primary-color)]">
            바로 적용 →
          </span>
        </button>
        ...
        <div className="mt-4">
          <SectionRow
            label="로스팅 단계"
            onClick={openBottomSheet}
            value={detail.defaultRoastLabel}
          />
          <SectionRow
            label="분쇄 옵션"
            onClick={openBottomSheet}
          ...
          <SectionRow
            label="용량"
            onClick={openBottomSheet}
            value={`${detail.baseWeightLabel} — ₩${formatPrice(product.price)}`}
          />
        </div>
      </section>
      ...
      <section className="border-b border-[var(--line-color)] bg-white px-4 py-4">
        ...
        >
          <div>
            <h2 className="text-[1rem] font-bold text-[var(--palette-121212)]">
              브루잉 가이드
            </h2>
            <p className="mt-1 text-[0.7rem] text-[var(--palette-666666)]">
              {detail.brewingGuide}
            </p>
          </div>
      ...
      <section className="border-b border-[var(--line-color)] bg-white px-4 py-4">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {detail.noteLabels.map((note, index) => (
            <span
              key={note}
        ...
        <p className="mt-4 text-[0.86rem] leading-7 text-[var(--palette-121212)]">
          {product.description} {detail.descriptionSuffix ?? ""}
        </p>
      </section>
      ...
      <section className="border-b border-[var(--line-color)] bg-white px-4 py-4">
        ...
        <div className="mt-3 space-y-1 text-[0.86rem] leading-7 text-[var(--palette-121212)]">
          {detail.storyLines.map((line, index) =>
            line ? (
      ...
      <section className="bg-white px-4 py-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[0.92rem] text-[var(--palette-666666)]">
            {reviewTotal}
          </span>
        </div>

        <div className="mt-2 flex items-end gap-2">
          <p className="text-[1.55rem] font-black text-[var(--accent-color)]">
            ★ {product.rating?.toFixed(1) ?? "4.8"}
          </p>
          <p className="pb-1 text-[0.9rem] text-[var(--palette-666666)]">
            / 5.0
          </p>
        </div>
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <span className="text-[0.8rem] text-[var(--accent-color)]">
                    {getRatingText(review.rating)}
                  </span>
                  <span className="text-[0.72rem] font-medium text-[var(--palette-121212)]">
                    {review.author}
                  </span>
                </div>
                <span className="text-[0.66rem] text-[var(--palette-666666)]">
                  {review.date}
                </span>
              </div>
              <p className="mt-2 text-[0.82rem] leading-6 text-[var(--palette-121212)]">
                {review.content}
              </p>
              <p className="mt-2 text-[0.66rem] text-[var(--palette-666666)]">
                {review.optionLabel}
              </p>
            </article>
          ))}
      ...
      <div className="fixed bottom-0 left-1/2 z-30 w-full max-w-md -translate-x-1/2 border-t border-[var(--line-color)] bg-white px-4 py-2.5">
        <div className="mb-2 flex items-center justify-between text-[0.7rem]">
          <span className="text-[var(--palette-666666)]">
            선택된 옵션: {selectedSummary}
          </span>
          <span className="font-bold text-[var(--danger-color)]">
        ...
        <div className="grid grid-cols-[1fr_1.12fr] gap-2">
          ...
          <PrimaryButton
            className="h-11 rounded-[0.85rem] shadow-none"
